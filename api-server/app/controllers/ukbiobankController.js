const Data = require('../models/ukbiobankModel.js');
...
function groupConditions(conditions) {
  ...
  const queryConditions = parsedConditions.map(
    (condition, index) => {
      ...
      const splitConditionE = condition.split(relationDelimiters)[1]
      if (!isNaN(splitCondition)) {
        const match = condition.match(relationDelimiters)
        curCondition = '"' + splitCondition + '"' + match[0] + splitConditionE;
      }
      const splitCurCondition = curCondition.split(relationDelimiters)[0]
      const splitCurConditionE = curCondition.split(relationDelimiters)[1]
      if (curCondition.includes('NOTNULL')) {
        curCondition =  splitCurCondition + ' IS NOT NULL'
      }
      if (index === 0) {
...
exports.get_data = async function (req,res) {
  try {
    ...
    console.log(conditions);
    const result = await Data.getData(values,conditions);
...
exports.get_range = async function (req,res) {
  try {
    ...
    const result = await Data.getRange(value);
...
exports.get_query = async function (req,res) {
  try {
    ...
    const result = await Data.getQuery(values,conditions);
